...
const validar = [
    body('Nombre_act')
        .isLength({ min: 5, max: 15 }).withMessage('El nombre de la actividad debe tener entre 5 y 15 caracteres'),
...
const registrarActividad = async (req, res) => {
    if (req.headers.authorization) {
        try {
            ...
            if (!admin) {
                // Si no se encuentra un administrador con el ID y el token proporcionados
