...
const renderAllNino = async (req, res) => {
    try {
        ...
        // Verificar si el token corresponde a un usuario tutor en la base de datos
...
const loginNino = async (req, res) => {
    ...
    if (!NinoBDD) return res.status(401).json({ msg: "Lo sentimos, el niño no se encuentra registrado" })
...
const perfilNino = async (req, res) => {
    try {
        ...
        try {
            ...
            const nino = await Nino.findById(id).select("-Password_nino -token -updatedAt -status -confirm_tutor -createdAt -__v -_id");
            // Validar si existe el niño
            if (!nino) {
...
const comprobarTokenPaswordNino = async (req, res) => {
    ...
    if (ninoBDD?.token !== req.params.token) return res.status(400).json({ msg: "Lo sentimos, no se puede validar la cuenta" })
    // Guardar en BDD
    await ninoBDD.save()
